...
### View and Redirect Helpers

The framework provides convenient global helpers for rendering views and handling redirects:

**Using `view()` helper:**

```typescript
// Render a jsBlade view
return view("welcome", {
  title: "Welcome",
  name: "John",
});
...
// Inertia redirect
return inertiaRedirect("/users", "User created!", "success");
```

**Complete example using view and redirect helpers:**
...
async store() {
  ...
  const user = await User.create(await request().validated());

  if (request().expectsJson()) {
    return response().json({ user });
  }

  // Use redirect helper with flash message
  return redirect("/users").with("success", "User created!");
}
...
**Note:** These helpers automatically access the current HTTP response context, making it easy to return views and redirects without explicitly accessing the `res` object. They work seamlessly with both traditional server-side rendering and Inertia.js applications.
...
// Get previous URLs
const previousUrls = req.previousUrls;
```
...
Scopes allow you to define reusable query constraints in your models. There are two types: **local scopes** (called manually) and **global scopes** (applied automatically).

### Local Scopes

Local scopes allow you to define common sets of query constraints that you may easily re-use throughout your application. To define a scope, add a static method prefixed with `scope` + StudlyCase name. Call them **without** the prefix when using.

**Naming convention:** `scopeActive` → call as `.active()`, `scopeType` → call as `.type("admin")`

```typescript
import { Model } from "jcc-eloquent";
import type { Builder } from "jcc-eloquent/lib/Builder";
...
class User extends Model {
  ...
  static scopeType(query: Builder, type: string) {
    return query.where("type", type);
  }

  static scopeRecent(query: Builder, days: number = 7) {
    const date = new Date();
...
**Usage:** The query builder uses a proxy that resolves scopes automatically. Call scopes without the `scope` prefix:
...
// Scope with argument
const admins = await User.query().type("admin").get();
...
**Key points:**
- Method name must start with `scope` (e.g., `scopeForUser`)
- Call without prefix: use `.forUser(1)` not `.scopeForUser(1)`
- First parameter is always `query: Builder`; return the query for chaining
- Scopes work after `clone()` — the cloned builder stays proxied

### Global Scopes

Global scopes allow you to add constraints to **all** queries for a given model. Define a scope class implementing the `Scope` interface and register it with the `@ScopedBy` decorator.

```typescript
import { Model, Scope, Builder, ScopedBy } from "jcc-eloquent";
...
class User extends Model {
  // All queries automatically have where('active', true)
}
```

You can also use plain functions:

```typescript
@ScopedBy([
  (builder) => builder.where("active", true),
  (builder) => builder.whereNotNull("deleted_at"),
])
class User extends Model {}
```

**Bypass global scopes:** Use `withoutGlobalScopes()` when you need to query without them:

```typescript
const allUsers = await User.query().withoutGlobalScopes().get();
```
...
### Laravel-style Content

Use the `Content` class to define view templates with custom data, similar to Laravel's Content definition:

```typescript
import { Mail, Content } from "jcc-express-mvc/Core/Mail";

// HTML only
await Mail.to("user@example.com").send({
  content: new Content({
    view: "views.email.welcome",
    with: { user, url: "/dashboard" },
  }),
  subject: "Welcome",
...
**Content options:**
- `view` / `html` - Main HTML template path (dot notation, e.g. `views.email.welcome`)
- `text` - Optional plain-text template path
- `with` - Data passed to the templates

### Laravel-style Mailable Classes

Create reusable email classes by extending `Mailable`, implementing `content()` and `envelope()`:
...
import {
  ...
  Address,
  Attachment,
} from "jcc-express-mvc/Core/Mail";
import { config } from "jcc-express-mvc/lib/Config/Config";

class OrderShipped extends Mailable {
  constructor(protected order: { name: string; price: number }) {
    super();
  }

  content(): Content {
    return new Content({
      view: "views.mail.orders.shipped",
      with: {
  ...
  envelope(): Envelope {
    return new Envelope({
      subject: "Order Shipped",
      from: new Address(
        config.get("MAIL_FROM_ADDRESS", "noreply@example.com"),
        config.get("MAIL_FROM_NAME", "Store"),
      ),
    });
  }
...
### Laravel-style Attachments

Use the `Attachment` class for a fluent, Laravel-style attachment API:
...
// From Storage (local disk)
Attachment.fromStorage("invoices/2024.pdf");
Attachment.fromStorageDisk("local", "invoices/2024.pdf");

// From in-memory data
Attachment.fromData(() => pdfBuffer, "report.pdf");
Attachment.fromData(async () => await fetchPdf(), "report.pdf");
```

**Attachment methods:**
- `fromPath(path)` - Attach from filesystem path
- `fromStorage(path, disk?)` - Attach from Storage default disk
- `fromStorageDisk(disk, path)` - Attach from a specific Storage disk
- `fromData(dataFn, filename)` - Attach from callback returning Buffer or string
- `.as(name)` - Set display filename
- `.withMime(mime)` - Set MIME type (e.g. `"application/pdf"`)

## Mailer Interface

The `send()` method accepts a `Mailer` object or a `Mailable` instance:
...
interface Mailer {
  ...
  content: string | Content; // View path (string) or Laravel-style Content definition
  from?: string;            // Override from address (e.g. "Name <email@example.com>")
...
**Supported drivers:** SMTP (Nodemailer), Resend. Set `MAIL_DRIVER=smtp` or `MAIL_DRIVER=resend` in `.env`.
...
# Create a new API controller (resourceful)
bun artisanNode make:api-controller UsersController
...
bun artisanNode make:migration create_users_table
bun artisanNode make:migration create_users_table table=users
...
bun artisanNode make:listener SendShipmentNotification event=OrderShipped
...
bun artisanNode make:command SendEmails signature=emails:send
...
# Rollback last batch (or specified steps)
bun artisanNode migrate:rollback
bun artisanNode migrate:rollback steps=2
...
# List routes filtered by middleware
bun artisanNode route:list middleware=api
```

### Schedule Commands

```bash
# Run scheduled tasks (runs continuously)
...
```bash
# Start the Inertia.js SSR server
...
In addition to the commands provided by ArtisanNode, you may also build your own custom commands. Commands are stored in the `app/Console/Command` directory and are automatically discovered by the framework.

### Generating Commands

To create a new command, use the `make:command` ArtisanNode command:

```bash
# Create a command (signature auto-derived from name: SendEmails -> send:emails)
bun artisanNode make:command SendEmails

# Create a command with a custom signature
bun artisanNode make:command SendEmails signature=emails:send
```

The command will create a new file in `app/Console/Command/` (e.g. `SendEmailsCommand.ts`). The class name is automatically suffixed with `Command` if not provided.

### Command Structure

Every command extends the base `Command` class and defines a `signature`, `description`, and `handle` method:

```typescript
import { Command } from "../../../jcc-express-mvc/Core/Console/Command";

export class SendEmailsCommand extends Command {
  signature = "send:emails";
  description = "Send queued emails to users";

  async handle(firstArg?: string, secondArg?: string) {
...
The `Command` base class provides helper methods for output:

- `this.info(message)` - Blue informational output
- `this.success(message)` - Green success output
- `this.error(message)` - Red error output

### Running Commands

Once created, your command is automatically registered. Run it using its signature:

```bash
bun artisanNode send:emails
```

### Dependency Injection

Commands support dependency injection via the `@Method()` decorator. Inject services from the container into your `handle` method:

```typescript
import { Command } from "../../../jcc-express-mvc/Core/Console/Command";
import { Method } from "../../../jcc-express-mvc/Core/Dependency";

export class ProcessOrdersCommand extends Command {
  signature = "orders:process";
  description = "Process pending orders";

  @Method()
  async handle(orderService: OrderService) {
    await orderService.processPending();
    this.success("Orders processed!");
  }
}
```
...
The `handle` method receives `firstArg` and `secondArg` as positional arguments from the command line. For more complex argument parsing, you can use the optional `signature=` parameter when generating, or parse the arguments manually in your handle method.
